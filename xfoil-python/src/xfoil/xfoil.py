...
# import sys
# sys.path.insert(0, '/Users/Kike/Library/Mobile Documents/com~apple~CloudDocs/Documents/Inbox/TU_Delft/MsC/Q1/Aircraft_Aerodynamics/Code/xoil-python/')


import ctypes
import glob
import os
from ctypes import POINTER, byref, c_bool, c_char, c_char_p, c_float, c_int, cdll
...
class XFoil(object):
    ...
    def a(self, a):
        ...
        self._lib.alfa(
            byref(c_float(a)), byref(cl), byref(cd), byref(cd_f), byref(cd_p), byref(cm), byref(cp), byref(conv)
        )

        return (
            (cl.value, cd.value, cd_f.value, cd_p.value, cm.value, cp.value)
            if conv
            else (np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
        )
    ...
    def cl(self, cl):
        ...
        self._lib.cl(
            byref(c_float(cl)), byref(a), byref(cd), byref(cd_f), byref(cd_p), byref(cm), byref(cp), byref(conv)
        )

        return (
            (a.value, cd.value, cd_f.value, cd_p.value, cm.value, cp.value)
            if conv
            else (np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
        )
